...
class CounterBalanceSheetsController extends AppController {
	 ...
	 */
	 public function index() {		
		$conditions = array('CounterBalanceSheet.store_id'=>$this->Session->read('Store.id'));					
		$this->paginate = array(
							'conditions' => $conditions,
							'order' => array('CounterBalanceSheet.from_date' => 'DESC', 'CounterBalanceSheet.created' => 'DESC'),
							'limit' => 100,
							'recursive' => '-1'
							);
	...
	function addCounterBalanceSheetFormValidation($data=null) {
		$error = null;
	...
	function add() {
		...
		App::uses('Sale', 'Model');
		$this->Sale = new Sale;
		$conditions = array('Sale.store_id'=>$this->Session->read('Store.id'), 'Sale.sale_date >='=>$fromDate, 'Sale.sale_date <='=>$toDate);				
		$sales = $this->Sale->find('all', array('conditions'=>$conditions, 'order'=>'Sale.sale_date', 'recursive'=>'-1'));
		$sale_amount = 0;
		if($sales) {
		...
		$conditions = array('Cashbook.store_id'=>$this->Session->read('Store.id'), 'Cashbook.payment_date >='=>$fromDate, 'Cashbook.payment_date <='=>$toDate);				
		$cashbook_records = $this->Cashbook->find('all', array('conditions'=>$conditions, 'order'=>'Cashbook.payment_date', 'recursive'=>'-1'));
		$expense_amount = 0;
		if($cashbook_records) {
		...
		$conditions = array('TransactionLog.store_id'=>$this->Session->read('Store.id'), 'TransactionLog.payment_date >='=>$fromDate, 'TransactionLog.payment_date <='=>$toDate);
		$logs = $this->TransactionLog->find('all', array('conditions'=>$conditions, 'order'=>'TransactionLog.payment_date'));
		...
		if($this->request->isPost() or $this->request->isPut()) {
			...
			if($data['CounterBalanceSheet']['save_data']) {
				...
				if($this->CounterBalanceSheet->save($data)) {
					...
					$this->redirect(array('controller'=>'CounterBalanceSheets', 'action'=>'index'));
				} else {
		...
		$conditions = array('CounterBalanceSheet.store_id'=>$this->Session->read('Store.id'));
		$sheets = $this->CounterBalanceSheet->find('all', array('conditions'=>$conditions, 'order'=>'CounterBalanceSheet.created DESC', 'recursive'=>'-1', 'limit'=>'10'));
