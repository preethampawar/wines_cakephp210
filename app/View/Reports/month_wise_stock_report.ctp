...
?>
	...
	<?php if($showForm) { ?>
		...
		<div id="paramsDiv">	
			...
			<div style="float:left; clear:none;">
				<?php echo $this->Form->input('month', array('label'=>'Month', 'type'=>'month', 'empty'=>false, 'default'=>$month));?>
			</div>
			<div style="float:left; clear:none;">
				<?php 
				$options = array(date('Y')=>date('Y'), (date('Y')-1)=>(date('Y')-1), (date('Y')-2)=>(date('Y')-2), (date('Y')-3)=>(date('Y')-3));
				echo $this->Form->input('year', array('label'=>'Year', 'type'=>'select', 'empty'=>false, 'options'=>$options, 'default'=>$year));
				?>
			</div>
			...
			<div style="float:left; clear:none; padding-top:15px;">
				<?php echo $this->Form->submit('Get Report', array('id'=>'SubmitForm', 'title'=>'', 'type'=>'submit', 'onclick'=>'return submitButtonMsg()'));?>
			</div>
