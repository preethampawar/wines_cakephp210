...
?>
	...
	?> 	
		...
		<div id="paramsDiv">
			...
			<div style="float:left; clear:none;">
				<?php echo $this->Form->input('payment_type', array('label'=>'Payment Type', 'empty'=>'-- All --', 'type'=>'select', 'options'=> array('expense'=>'Payment Made','income'=>'Payment Received'), 'default'=>'expense'));?>
			</div>
			<div style="float:left; clear:none;">
				<?php echo $this->Form->input('tag_id', array('label'=>'Tag', 'empty'=>'-- All --', 'type'=>'select', 'options'=>$tags, 'class'=>'autoSuggest'));?>
			</div>
	...
	$csv.="\r\n";
	if(!empty($logs)) {	
		$csv.=implode(array('#', 'Payment Date', 'Tag', 'Description', 'Payments Made', 'Payments Received'), ",")."\r\n";
		$k=0;
		$total_payments_made = 0;
		$total_payments_received = 0;
