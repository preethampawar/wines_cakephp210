...
class BanksController extends AppController
{
	...
	public function index()
	{
		...
		$conditions = ['Bank.store_id' => $this->Session->read('Store.id')];

		$this->paginate = [
			'conditions' => $conditions,
			'order' => ['Bank.payment_date' => 'DESC', 'Bank.created' => 'DESC'],
			'limit' => 100,
			'recursive' => '-1',
		];
	...
	public function add($categoryID = null)
	{
		...
		if ($this->request->isPost() or $this->request->isPut()) {
			...
			if (!$error) {
				...
				$conditions = ['Bank.payment_date' => $paymentDate, 'Bank.amount' => $data['Bank']['amount'], 'Bank.title' => $data['Bank']['title']];
			}
			...
			if (!$error) {
				...
				if ($this->Bank->save($data)) {
					...
					$this->redirect(['controller' => 'banks', 'action' => 'add']);
				}
			}
		} else {
			if ($this->Session->check('paymentDate')) {
	...
	public function addBankFormValidation($data = null)
	{
		$error = null;
	...
	public function remove($recordID = null)
	{
		if ($this->request->isPost()) {
			if ($bankInfo = $this->Bank->findById($recordID)) {
				...
				$this->Session->setFlash('Bank information, dated "' . date('d M Y', strtotime($bankInfo['Bank']['payment_date'])) . '" has been removed from the list', 'default', ['class' => 'success']);
			} else {
