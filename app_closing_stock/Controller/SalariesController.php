...
class SalariesController extends AppController
{
	...
	{
		$conditions = ['Salary.store_id' => $this->Session->read('Store.id')];
		$this->paginate = [
			'conditions' => $conditions,
			'order' => ['Salary.payment_date' => 'DESC', 'Salary.created' => 'DESC'],
			'limit' => 100,
			'recursive' => '-1',
		];
	...
	public function add()
	{
		...
		$employeesList = $this->Employee->find('list', ['conditions' => $conditions, 'order' => 'Employee.name', 'recursive' => '-1']);
		...
		if ($this->request->isPost() or $this->request->isPut()) {
			...
			if (!$error) {
				if (!$employeeInfo = $this->CommonFunctions->getEmployeeInfo($employeeID)) {
					...
					$conditions = ['Salary.employee_id' => $employeeID, 'Salary.payment_date' => $paymentDate, 'Salary.payment_amount' => $data['Salary']['payment_amount']];
					if ($this->Salary->find('first', ['conditions' => $conditions])) {
			...
			if (!$error) {
				...
				if ($this->Salary->save($data)) {
					...
					$this->redirect(['controller' => 'salaries', 'action' => 'add']);
				}
			}
		} else {
		...
		$conditions = ['Salary.store_id' => $this->Session->read('Store.id')];
		$salaries = $this->Salary->find('all', ['conditions' => $conditions, 'order' => 'Salary.created DESC', 'recursive' => '-1', 'limit' => '10']);
	...
	public function addSalaryFormValidation($data = null)
	{
		$error = null;
	...
	public function remove($salaryID = null)
	{
		if ($this->request->isPost()) {
			if ($salaryInfo = $this->Salary->findById($salaryID)) {
				...
				$this->Session->setFlash('"' . $salaryInfo['Salary']['employee_name'] . '" Salary information, dated "' . date('d M Y', strtotime($salaryInfo['Salary']['payment_date'])) . '" has been removed from the list', 'default', ['class' => 'success']);
			} else {
