...
class TransactionLogsController extends AppController
{
	...
	{
		$conditions = ['TransactionLog.store_id' => $this->Session->read('Store.id')];

		$this->paginate = [
			'conditions' => $conditions,
			'order' => ['TransactionLog.payment_date' => 'DESC', 'TransactionLog.created' => 'DESC'],
			'limit' => 100,
			'recursive' => '-1',
		];
		$logs = $this->paginate();
	...
	public function add()
	{
		...
		if ($this->request->isPost() or $this->request->isPut()) {
			...
			if (!$error = $this->addTransactionLogFormValidation($data)) {
				...
				if ($this->TransactionLog->save($data)) {
					...
					$this->redirect(['controller' => 'TransactionLogs', 'action' => 'add']);
				} else {
		...
		$conditions = ['TransactionLog.store_id' => $this->Session->read('Store.id')];
		$logs = $this->TransactionLog->find('all', ['conditions' => $conditions, 'order' => 'TransactionLog.created DESC', 'recursive' => '-1', 'limit' => '10']);
	...
	public function addTransactionLogFormValidation($data = null)
	{
		$error = null;
	...
	public function addTag()
	{
		...
		if (isset($this->request->data) and !empty($this->request->data)) {
			...
			if (!empty($data['Tag']['name'])) {
				...
				$conditions = ['Tag.name' => $data['Tag']['name'], 'Tag.store_id' => $this->Session->read('Store.id')];
				if ($this->Tag->find('first', ['conditions' => $conditions])) {
			...
			if (!$error) {
				...
				if ($this->Tag->save($data)) {
					$this->Session->setFlash('Tag "' . $data['Tag']['name'] . '" Created Successfully', 'default', ['class' => 'success']);
				} else {
