	...
	<table class='table' style="width:100%">
		...
		foreach ($salaries as $row) {
			...
			<tr>
				...
				<td>
					<form method="post" style="" name="salary_<?php echo $row['Salary']['id']; ?>"
						  id="salary_<?php echo $row['Salary']['id']; ?>"
						  action="<?php echo $this->Html->url("/salaries/remove/" . $row['Salary']['id']); ?>">
						<input type="submit" value="Remove" name="Remove"
							   onclick="if (confirm('Are you sure you want to delete this record of employee - <?php echo $row['Salary']['employee_name']; ?> from the list?')) { $('#salary_<?php echo $row['Salary']['id']; ?>').submit(); } event.returnValue = false; return false;">
					</form>
